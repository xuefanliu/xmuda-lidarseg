...
def preprocess(nusc, split_names, root_dir, out_dir,
    ...
    for i, sample in enumerate(nusc.sample):
        ...
        #########################################################################################
        #    这部分是处理boxes，将框框里的点的标签打上，没有在框框里的点标签为10
        #########################################################################################
        # only use lidar points in the front camera image，这里是原始点云，不是与图像空间校准过后的点云哦
        pts = pts[:, pts_valid_flag]  # (3,N')
        num_pts = pts.shape[1]
        seg_labels = np.full(num_pts, fill_value=len(class_names_to_id), dtype=np.uint8)  # 标签数组，此处全部填的10
        ...
        for box in boxes:
            ...
            fg_mask = points_in_box(box, pts)  # bool数组
