...
def map_pointcloud_to_image(pc, im_shape, info, im=None):
    ...
    rotation = Quaternion(info['ego2global_rotation_cam']).rotation_matrix
    pc = pc - np.array(info['ego2global_translation_cam'])[:, np.newaxis]  # cam_ego->global needs to add+, so global->cam_ego needs minus-
    pc = Quaternion(info['ego2global_rotation_cam']).rotation_matrix.T @ pc # cam_ego->global 需要乘一个旋转矩阵，so global->cam_ego乘矩阵的转置
    ...
    points = view_points(pc, np.array(info['cam_intrinsic']), normalize=True)  # 点云乘以相机校准，然后进行正则化
